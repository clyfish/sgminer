...
built:
	...
	applog(LOG_NOTICE, "Initialising kernel %s with%s bitalign, %spatched BFI, %d vectors, worksize %d, N %d (N factor %d)",
	       filename, clState->hasBitAlign ? "" : "out", patchbfi ? "" : "un", clState->vwidth, (int)(clState->wsize),
	       algorithm->n, algorithm->nfactor);
